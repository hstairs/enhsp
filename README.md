...
This repository contains ENHSP, which stands for Expressive Numeric Heuristic Planner. It is, of course, a heuristic search planner, but it is expressive in that it can handle:

1. Classical Planning
2. Numeric Planning with linear and non-linear (!!) expressions
3. Planning with discretised autonomous processes
...
The planner reads in input a PDDL domain and problem file, and if you are lucky and your problem is not too complex, it provides you with a plan (a sequence of actions). In the case of planning with processes, the plan is a time-stamped plan (associated to each action, you find the time at which that instance of the action has to be executed). In dealing with processes, ENHSP discretises the problem (with a delta=1sec by default); so the plan is guaranteed to be valid only with respect to that discretisation.

The input language for the planner is PDDL. PDDL is the standard de facto language to express planning problems. The domain file expresses the signature of your predicates, functions and all the actions/processes available, in a parametrized way. The problem file expresses the particular instance of the planning problem (e.g., what is the initial value of predicate A? What is the goal?). For more information on PDDL I suggest you to start from [its wikipedia page](https://en.wikipedia.org/wiki/Planning_Domain_Definition_Language#cite_note-opt-15), and follow the links. ENHSP supports PDDL 2.1 in particular, and PDDL+ (for the support of autonomous processes) but without events (we plan to support them soon). We also allow to employ global constraint as a direct construct of the language (via the :constraint syntax).

The planner has been developed taking ideas from different papers as for instance: 
...
ENHSP depends on a number of other libraries, some for the PDDL parsing, other for some standard algorithm on graphs, and some to interface the API with a Linear Program solver:
...
They are all open source projects, so is this library. For your convenience, *the necessary jar files are all within the ``libs`` folder*. 
...
```
./enhsp -o <domain_file> -f <problem_file> -planner <string> (options: ssnp_sat, ssnp_opt, hp_all, hp_rp)
```
domain_file and problem_file are the PDDL input files.

- ssnp_sat and ssnp_opt are the configurations for the satisficing and the optimal version of ENHSP. They differ on the use of the search strategy and the heuristic employed. They are made specifically for sequential numeric planning without autonomous processes (even though they can work with processes as well, in some case). These two planners have been developed mainly to test the ideas reported in the IJCAI 2016 paper.

- hp_all, hp_rp are the configurations for planning with autonomous processes. Both use A* as search engine; whereas hp_all uses a pure AIBR heuristic as for the ECAI 2016 paper, hp_rp uses a variation of that. It extracts a relaxed plan from the relaxed problem (this last configuration is not published, and is still under experimental analysis).

The things above are not completely true. 'hp_all' and 'hp_rp' can be used for general numeric planning problems as described in ECAI 2016. 'ssnp_sat' and 'ssnp_opt' provides guarantees (speed and quality of the solutions) for restricted class of numeric planning.

The planner can be also run using highly customized settings, which allow you to combine heuristic and search engine in an ad-hoc fashion. It may be useful to do so if your problem exhibits particular structures.
...
Besides the examples, there are also domains and instances used to test the planner in the two papers aforementioned:

##Ecai 2016

The instances used for ECAI 2016 are in the ecai16_benchmarks folder. The planner used for those instances is 'hp_all' which corresponds to a setting using A* plus the pure AIBR heuristic used in the paper. Note that this does not mean that this planner cannot obtain *better* performances using other settings of parameters. The analysis performed in the ECAI 2016 paper was intended *just to measure the effectiveness of the heuristic and of the relaxation behind it*

##Ijcai 2016

The instances used for ECAI 2016 are in the ecai16_benchmarks folder. The planner used for those instances is:

- 'ssnp_sat' which is GBFS plus the numeric inadmissibile additive h_{add} heuristic
- 'ssrnp_sat' which is GBFS plus the numeric inadmissibile additive h_{add} heuristic using also redundant constraints
- 'ssnp_opt' which is A^* and the numeric admissibile h_{max} heuristic using also redundant constraints
...
1. No support for conditional effects, quantification over objects
2. The 'ssnp_sat' and 'ssrnp_sat' version do not accept negative precondition. You can compile them away but it would be nice to have native support for them
3. The effect set is assumed to be given always as an AND condition, even when there is just a single effect. So if you have one single effect e_1, you can obtain an equivalent formulation by writing (and e_1) 
4. ...
